(function(factory) {
  ...
  if (typeof window !== 'undefined') {
    ...
    window.app.casePageAiGenTaskRunner = api;
  }
})(function() {
  ...
  function defaultFileMeta(caseFile) {
    var file = caseFile && typeof caseFile === 'object' ? caseFile : {};
    return {
      id: file.id || null,
      name: file.file_name_clean || file.name || '',
      projectId: file.project_id || file.projectId || null,
      versionId: file.version_id || file.versionId || null,
    };
  }
  ...
  function create(options) {
    ...
    if (!aiGenModel || typeof aiGenModel.parseResult !== 'function') {
      throw new Error('case page AI generation task runner model is required');
    }
    var scene = opts.scene || 'case-library';
    var runTokenPrefix = opts.runTokenPrefix || 'local';
    ...
    var resolveCoverageThreshold = typeof opts.resolveCoverageThreshold === 'function'
      ...
      : function() { return 90; };
    var resolveFileMeta = typeof opts.resolveFileMeta === 'function' ? opts.resolveFileMeta : defaultFileMeta;
    ...
    function resume(force) {
      ...
      try {
        if (task && task.status === 'running' && typeof manager.startTask === 'function') {
          manager.startTask(scene, task, { force: force === true });
        } else if (typeof manager.resumeTasks === 'function') {
          manager.resumeTasks({ force: force === true });
        }
      } catch (err) {
        // Ignore task-manager resume failures and leave the current snapshot usable.
      }
      return getTask() || task;
    }

    function getCurrentTask(currentFileId) {
      var task = getTask();
      if (!task || task.scene !== scene) return null;
      var taskFileId = task.caseFileId ? String(task.caseFileId) : '';
      var fileId = currentFileId ? String(currentFileId) : '';
      if (!taskFileId || !fileId || taskFileId !== fileId) return null;
      return task;
    }
    ...
    function getRunningConflict(currentFileId) {
      ...
      if (activeFileId === currentId) return { type: 'same-file', task: task };
      return {
        type: 'other-file',
        task: task,
    ...
    function buildXmindPipeline(prepApi, prepContext, fileMeta, requirementText, moduleList, casePayload, threshold) {
    ...
    function prepare(input) {
      ...
      if (!caseFile) throw createRunnerError('no-case', '请先选择用例');
      ...
      var fileMeta = resolveFileMeta(caseFile) || defaultFileMeta(caseFile);
      ...
      var signature = aiGenModel.buildSignature(fileMeta.id, requirementText, moduleList, prepContext);
      var runToken = runTokenPrefix + '-' + now().toString(36) + '-' + random().toString(36).slice(2, 6);
      return {
        caseFile: caseFile,
        fileMeta: fileMeta,
        items: items,
        ...
        signature: signature,
        runToken: runToken,
        generationMode: aiGenModel.resolveGenerationMode(prepContext),
      };
    }

    function createTaskPayload(prepared) {
      var fileMeta = prepared.fileMeta || defaultFileMeta(prepared.caseFile);
      return {
        contextSignature: prepared.signature,
        caseFileId: fileMeta.id || null,
        caseFileName: fileMeta.name || '',
        projectId: fileMeta.projectId || null,
        versionId: fileMeta.versionId || null,
        versionIdAtRun: fileMeta.versionId || '',
        versionAssigned: Boolean(fileMeta.versionId),
    ...
    function start(prepared, startOptions) {
      ...
      if (manager && typeof manager.createTask === 'function' && typeof manager.startTask === 'function') {
        var task = manager.createTask(scene, createTaskPayload(prepared));
        manager.startTask(scene, task);
    ...
    function resolveManagedResult(task, context) {
      ...
      if (taskId && resultByTaskId[taskId]) return { kind: 'ready', parsed: resultByTaskId[taskId] };
      var parsed;
      try {
    ...
    function clear(taskId) {
      ...
      if (pendingByTaskId[key]) revisionByTaskId[key] = Number(revisionByTaskId[key] || 0) + 1;
      else delete revisionByTaskId[key];
  ...
  return {
    ...
    createRunnerError: createRunnerError,
  };
});
