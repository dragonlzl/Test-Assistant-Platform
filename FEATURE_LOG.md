...
- 功能名称：用例生成跨模块勾选导出 XMind  
- 功能描述：用例生成页新增“导出所选用例XMind”全局按钮，支持在各模块用例视图勾选后一次性导出合并 XMind；导入模块用例后自动解锁导出入口。  
- 操作方式：在模块“用例视图”勾选用例后关闭抽屉，点击用例生成页顶部“导出所选用例XMind”即可下载，文件名沿用需求标识+时间戳；导入模块用例（带 `#CASE_MODULE` 标识）后，导出全部/单模块按钮自动可用。  
- 使用效果：一次导出多模块选中用例，避免逐模块单独下载；导入调试文件后无需再次生成即可直接导出。  
- 新增内容/接口/组件：新增全局导出按钮 DOM/别名，casesGenCore 暴露 `exportSelectedModulesToXmind` 与按钮启用刷新；导入模块用例完成后刷新导出按钮；UI 用例 `tests/ui/casegen_export_xmind.spec.js` 增补全局导出覆盖。  
- 复用说明：复用现有 XMind 构建、需求标识与勾选收集逻辑，仅补充聚合导出与按钮状态联动。  
- 测试与验证：`npm run test:ui -- tests/ui/casegen_export_xmind.spec.js`（需提权启动本地 8090 http.server，已通过）。  
- 更新记录：无  
...
- 操作方式：在“需求澄清（人工确认）”点击“展开澄清视图”手动打开抽屉补充澄清；在“清洗后需求覆盖率”点击“覆盖缺失视图”查看缺失点、填写说明并继续流程；在“用例缺失测试点”点击入口打开抽屉查看缺失模块、复制/智能填充或跳转生成用例。勾选“需要人工确认”仅暂停流程，不会自动弹出抽屉。  
