...
- 使用效果：历史 `auto`、`clean`、`xmind-casegen` 地址和已保存页签统一回退到 `casesgen`，不再展示或恢复废弃页面；已有 XMind 工作区继续恢复。
- 新增内容/接口/组件：新增保留生成运行时边界，统一装配 XMind、用例库/执行页内生成和易漏用例提醒任务管理器；未新增后端接口或数据库结构。
- 复用说明：保留 XMind 严重复用的共享生成核心、模型客户端、解析器、任务恢复和通用新建/追加入库接口；本轮不进行高风险彻底拆核。
- 测试与验证：
  - `git diff --check` 通过；全部本次修改的 JavaScript 通过 `node --check`。
  - 8 个 XMind、用例筛选和执行复用 Node 核心测试全部通过。
  - 导航、模型、设置和页签套件初跑 27/29 通过；两个旧单页入口假设修正后定向复跑通过。
  - XMind 首次创建、刷新恢复、新建入库和追加入库 UI 回归 4/4 通过。
  - 用例库内生成 UI 回归 3/3 通过；执行页内生成 UI 回归 6/6 通过。
  - 配置/模型/功能指派存储、XMind 无新端点、通用入库复用和执行集追加 API 回归 4/4 通过（`apitest.db`）。
- 更新记录：2026-08-07 完成旧流程可达能力清理、历史路由归一、废弃配置过滤和文档更新。
