...
- 功能名称：一键执行视图抽屉化  
- 功能描述：AI 一键执行页的澄清视图、覆盖缺失列表与用例缺失模块视图改为右侧抽屉展示，遮罩可点击关闭，导航与主体不再被移位。  
- 操作方式：在“一键执行”页勾选“需要人工确认”后点击“展开澄清视图”打开抽屉；覆盖率不足时点击“覆盖缺失列表”查看缺失需求；缺失模块视图按钮打开用例缺失明细抽屉，均可通过遮罩或“收起”关闭。  
- 使用效果：抽屉承载长列表，背景保持可见且滚动位置不变，避免大视图撑开主页面；澄清/缺失视图开合一致，关闭后状态提示清空。  
- 新增内容/接口/组件：新增 autoClarify/autoCompare/autoMissing 抽屉 DOM 与开合逻辑，覆盖缺失/缺失模块按钮接入 drawer 组件，新增 UI 用例 `tests/ui/auto_drawer.spec.js`。  
- 复用说明：复用通用抽屉组件与既有澄清/缺失渲染逻辑，仅调整承载容器和滚动锁定方式。  
- 测试与验证：`npm run test:ui -- tests/ui/auto_drawer.spec.js`、`npm run test:ui -- tests/ui/drawer_nav_visibility.spec.js`（通过）。  
- 更新记录：无  
