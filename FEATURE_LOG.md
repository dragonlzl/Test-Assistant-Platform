...
- 更新记录：已初始化 FastAPI+SQLite 后端骨架（静态托管、默认管理员、鉴权/用户/项目/版本与分配、操作日志写入），新增用例库导入/编辑与执行集创建/用例写库等 API；前端增加登录浮层、后端鉴权接入、侧边栏显示当前用户、管理员页签（项目管理/人员管理/操作记录）占位与用例库入口；待前端详细接入、用例执行合并逻辑及模型/设置持久化补充。  
