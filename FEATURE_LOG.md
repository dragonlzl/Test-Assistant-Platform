...
- 更新记录：无  
- 功能名称：执行结果保留匹配精度提升  
- 功能描述：用例生成追加到执行文件时，仅当模块、用例标题与预期结果三者同时匹配旧用例时才继承原执行结果/备注/缺陷等信息，避免不同预期的用例误复用执行记录。  
- 操作方式：按需在用例生成页选择目标执行文件追加，用例匹配新增预期结果比对，无需额外操作。  
- 使用效果：同模块同标题但预期不同的用例不再继承旧执行记录，执行数据更可靠。  
- 新增内容/接口/组件：用例同步匹配键新增预期结果归一化处理。  
- 复用说明：复用既有模块/标题归一化与执行同步逻辑，仅扩展匹配条件。  
- 测试与验证：`npm run test:ui -- tests/ui/casegen_append_existing.spec.js`（通过，需 http.server 提权）。  
- 更新记录：无  
