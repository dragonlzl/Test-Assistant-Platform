...
test.describe('用例生成-设置项与跳转', () => {
  ...
  test('XMind 用例生成抽屉使用专属指派与提示词拼装', async ({ page }) => {
    ...
    expect(String(lastPrompt.prompt || '')).toContain('首次输出必须直接覆盖上述要求');
    expect(String(lastPrompt.prompt || '')).toContain('金额、积分、次数');
    expect(String(lastPrompt.prompt || '')).not.toContain('普通用例提示词');
    expect(String(lastPrompt.user || '')).toContain('【本轮生成选项(JSON)】');
    expect(String(lastPrompt.user || '')).toContain('【首轮生成硬约束】');
    expect(String(lastPrompt.user || '')).toContain('"needFunctionCondition": true');
    expect(String(lastPrompt.user || '')).toContain('"needNumericValidation": true');
    expect(String(lastPrompt.user || '')).toContain('"needBoundary": true');
    expect(String(lastPrompt.user || '')).toContain('"specialWeakNetwork": true');
    expect(String(lastPrompt.user || '')).toContain('已开启考虑功能使用条件');
    expect(String(lastPrompt.user || '')).toContain('解锁条件');
    expect(String(lastPrompt.user || '')).toContain('不允许把相关覆盖留到后续补全或追加');
    expect(String(lastPrompt.user || '')).toContain('XMind 页面专用要求');
  });
});
