...
test.describe('用例库编辑视图单条删除确认抽屉', () => {
  ...
  test('点击 - 删除用例时走确认抽屉', async ({ page }) => {
    const token = 'token-case-library-single-delete';
    const user = { id: 9, username: 'demo_admin', role: 'admin', level: 'leader' };
    const project = { id: 1, name: '项目A', description: 'for single delete' };
    ...
    await expect(toast).toContainText('已删除用例');
