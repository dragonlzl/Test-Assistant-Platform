...
function buildWaitingPayload(reviewJson) {
  return {
    model: {
      id: 'mock-model',
      name: 'mock',
...
test.describe('澄清等待恢复', () => {
  test('刷新恢复后继续等待澄清，不重复调用评审模型', async ({ page }) => {
    ...
    }, buildWaitingPayload(reviewJson));
  ...
  test('等待澄清时可切换到模型管理，不会被自动拉回一键执行', async ({ page }) => {
    await page.route('**/*', async (route) => {
      const url = route.request().url();
      if (url.indexOf('http://mock-model.local') === 0) {
        return route.fulfill({
    ...
    await page.evaluate(() => {
      if (window.app && typeof window.app.switchTab === 'function') {
        window.app.switchTab('models');
      }
    });
    await page.waitForURL(/\/ai-tools\.html(\?|$)/, { timeout: 10000 });
    await expect(page.locator('[data-tab-btn="models"]')).toHaveClass(/active/);

    // 等待自动流程恢复逻辑触发，确认不会又被拉回 AI 工作流页面。
    await page.waitForTimeout(2000);
    expect(page.url()).toContain('/ai-tools.html');
  });
});
