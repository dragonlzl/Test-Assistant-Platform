...
test.describe('＋新增用例高亮（绿色描边）', () => {
  ...
  test('执行页点击“＋”新增用例行有绿色描边（重渲染仍保留）', async ({ page }) => {
    const token = 'token-new-added-tempexec';
    const user = { id: 9, username: 'demo_admin', role: 'admin', level: 'leader' };
    const project = { id: 1, name: '项目A', description: 'for new added highlight' };
    ...
    await expect(page.locator('[data-temp-case-insert]').first()).toBeVisible();
    await page.locator('[data-temp-case-insert]').first().click();

    const newRows = page.locator('tr.case-row.new-added');
    await expect(newRows).toHaveCount(1);

    await newRows.locator('[data-temp-remark-toggle]').first().click();
    await expect(page.locator('tr.case-row.new-added')).toHaveCount(1);
  });

  test('用例库编辑页点击“＋”新增用例行有绿色描边（重渲染仍保留）', async ({ page }) => {
    const token = 'token-new-added-case-library';
    const user = { id: 9, username: 'demo_admin', role: 'admin', level: 'leader' };
    const project = { id: 1, name: '项目A', description: 'for new added highlight' };
    const versions = [{ id: 11, name: 'v1' }];
    const now = new Date().toISOString();
