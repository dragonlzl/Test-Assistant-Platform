const { test, expect } = require('@playwright/test');

test.describe('用例生成 Agent 设置', () => {
  const base = process.env.PLAYWRIGHT_BASE_URL || 'http://127.0.0.1:8090';
  const user = { id: 920, username: 'casegen_agent_user', role: 'admin', level: 'leader' };

  function createApiHandler(serverState, options) {
    const opts = options || {};
    let settingSeq = 10;
    return async function(route) {
      const url = new URL(route.request().url());
      const path = url.pathname;
      const method = route.request().method();
      const respond = (status, body) =>
        route.fulfill({ status, contentType: 'application/json', body: JSON.stringify(body) });

      if (path === '/api/users/me' && method === 'GET') return respond(200, user);
      if (path === '/api/settings' && method === 'GET') return respond(200, serverState.settings);
      if (path === '/api/settings' && method === 'PUT') {
        const body = route.request().postDataJSON() || {};
        const scope = body.scope || 'user';
        const items = Array.isArray(body.items) ? body.items : [];
        const now = new Date().toISOString();
        const saved = [];
        items.forEach((item) => {
          if (!item || !item.key) return;
          const ownerId = scope === 'global' ? null : user.id;
          let existing = serverState.settings.find(
            (row) => row.key === item.key && row.scope === scope && row.owner_id === ownerId
          );
          if (existing) {
            existing.value_json = item.value_json;
            existing.updated_at = now;
            saved.push(existing);
            return;
          }
          const next = {
            id: settingSeq += 1,
            scope,
            owner_id: ownerId,
            key: item.key,
            value_json: item.value_json,
            updated_at: now,
          };
          serverState.settings.push(next);
          saved.push(next);
        });
        return respond(200, saved);
      }
      if (path === '/api/projects' && method === 'GET') return respond(200, []);
      if (path.indexOf('/api/projects/') === 0 && path.indexOf('/versions') > -1 && method === 'GET') {
        return respond(200, []);
      }
      if (path === '/api/case-files' && method === 'GET') return respond(200, []);
      if (path === '/api/models' && method === 'GET') return respond(200, opts.models || []);
      if (path === '/api/features' && method === 'GET') return respond(200, []);
      if (path === '/api/ops' && method === 'GET') return respond(200, []);
      if (path === '/api/exec/overview' && method === 'GET') return respond(200, {});
      if (path === '/api/exec/overview/cases' && method === 'GET') return respond(200, []);
      if (path === '/api/exec/overview/layout' && method === 'GET') return respond(200, {});
      return respond(200, method === 'GET' ? [] : {});
    };
  }

  async function waitForAppReady(page) {
    await page.waitForFunction(() => window.app && window.app._inited === true, null, { timeout: 20000 });
  }

  test('未指派模型时无法开启 Agent', async ({ page }) => {
    const serverState = { settings: [] };
    const apiHandler = createApiHandler(serverState);

    await page.addInitScript(() => {
      try {
        localStorage.setItem('tap-auth-token', 'agent-toggle-token');
        localStorage.removeItem('usecase-settings-v1');
        localStorage.removeItem('cleaner-assignment-v1');
        localStorage.removeItem('cleaner-models-v1');
      } catch (err) {
        // ignore
      }
    });
    await page.route('**/*', (route) => {
      const target = route.request().url();
      if (target.startsWith('http://localhost') || target.startsWith('http://127.0.0.1') || target.startsWith('file:')) {
        return route.continue();
      }
      return route.abort();
    });
    await page.route('**/api/**', apiHandler);

    await page.goto(base + '/settings.html');
    await page.waitForLoadState('domcontentloaded');
    await waitForAppReady(page);
    const select = page.locator('#caseGenAgentEnabledSelect');
    const saveBtn = page.locator('#saveCaseGenAgentEnabled');
    await expect(select).toHaveValue('off');
    await select.selectOption('on');
    await saveBtn.click();
    await expect(select).toHaveValue('off');
    await expect.poll(() => {
      return serverState.settings.some((item) => item.key === 'caseGenAgentEnabled');
    }).toBe(false);
  });

  test('可保存 Agent 开关与覆盖率阈值', async ({ page }) => {
    const serverState = { settings: [] };
    const models = [
      {
        id: 'model-1',
        name: 'Agent Model',
        baseUrl: 'http://localhost:8081',
        model: 'deepseek-chat',
        apiKey: 'key-1',
      },
    ];
    const apiHandler = createApiHandler(serverState, { models });

    await page.addInitScript(() => {
      try {
        localStorage.setItem('tap-auth-token', 'agent-enabled-token');
        localStorage.removeItem('usecase-settings-v1');
        localStorage.setItem('cleaner-assignment-v1', JSON.stringify({ caseGenAgentId: 'model-1' }));
        localStorage.setItem('cleaner-models-v1', JSON.stringify([{
          id: 'model-1',
          name: 'Agent Model',
          baseUrl: 'http://localhost:8081',
          model: 'deepseek-chat',
          apiKey: 'key-1',
        }]));
      } catch (err) {
        // ignore
      }
    });
    await page.route('**/*', (route) => {
      const target = route.request().url();
      if (target.startsWith('http://localhost') || target.startsWith('http://127.0.0.1') || target.startsWith('file:')) {
        return route.continue();
      }
      return route.abort();
    });
    await page.route('**/api/**', apiHandler);

    await page.goto(base + '/settings.html');
    await page.waitForLoadState('domcontentloaded');
    await waitForAppReady(page);
    await page.evaluate(() => {
      if (!window.app || !window.app.state) return;
      window.app.state.models = [{
        id: 'model-1',
        name: 'Agent Model',
        baseUrl: 'http://localhost:8081',
        model: 'deepseek-chat',
        apiKey: 'key-1',
      }];
      if (!window.app.state.assignments) window.app.state.assignments = {};
      window.app.state.assignments.caseGenAgentId = 'model-1';
    });

    const select = page.locator('#caseGenAgentEnabledSelect');
    await expect(select).toHaveValue('off');
    await select.selectOption('on');
    await page.click('#saveCaseGenAgentEnabled');
    await expect(select).toHaveValue('on');

    await expect.poll(() => {
      const record = serverState.settings.find((item) => item.key === 'caseGenAgentEnabled');
      return record ? record.value_json : null;
    }).toBe('on');

    await page.fill('#caseGenAgentCoverageInput', '85');
    await page.click('#saveCaseGenAgentCoverage');

    await expect.poll(() => {
      const record = serverState.settings.find((item) => item.key === 'caseGenAgentCoverageThreshold');
      return record ? record.value_json : null;
    }).toBe(85);
  });
});
