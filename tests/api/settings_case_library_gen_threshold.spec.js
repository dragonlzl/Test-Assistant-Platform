...
test.describe('settings caseLibraryGenCoverageThreshold', () => {
  ...
  test('可写入并读取用例库生成覆盖度阈值', async () => {
    ...
    const saveFirst = await ctx.put(`${apiBase}/api/settings`, {
      headers,
      data: { scope: 'user', items: [{ key: 'caseLibraryGenCoverageThreshold', value_json: 88 }] },
    });
    ...
    const record = listBody.find((item) => item.key === 'caseLibraryGenCoverageThreshold');
    expect(record).toBeTruthy();
    expect(record.value_json).toBe(88);

    const saveSecond = await ctx.put(`${apiBase}/api/settings`, {
      headers,
      data: { scope: 'user', items: [{ key: 'caseLibraryGenCoverageThreshold', value_json: 95 }] },
    });
    ...
    const updated = listSecondBody.find((item) => item.key === 'caseLibraryGenCoverageThreshold');
    expect(updated).toBeTruthy();
    expect(updated.value_json).toBe(95);
  });
});
