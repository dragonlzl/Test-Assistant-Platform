...
test.describe('settings tempExecPageSize', () => {
  ...
  test('可写入并读取全局分页设置', async () => {
    ...
    const saveFirst = await ctx.put(`${apiBase}/api/settings`, {
      headers,
      data: { scope: 'user', items: [{ key: 'tempExecPageSize', value_json: 10 }] },
    });
    ...
    const record = listBody.find((item) => item.key === 'tempExecPageSize');
    expect(record).toBeTruthy();
    expect(record.value_json).toBe(10);

    const saveSecond = await ctx.put(`${apiBase}/api/settings`, {
      headers,
      data: { scope: 'user', items: [{ key: 'tempExecPageSize', value_json: 120 }] },
    });
    ...
    const updated = listSecondBody.find((item) => item.key === 'tempExecPageSize');
    expect(updated).toBeTruthy();
    expect(updated.value_json).toBe(120);
  });
});
