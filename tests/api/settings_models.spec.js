...
test.describe('settings/models/features + ops api', () => {
  ...
  test('settings, model configs, feature assignments scoped & permissioned', async () => {
    ...
    const saveExecSettings = await ctx.put(`${apiBase}/api/settings`, {
      ...
      data: {
        ...
        items: [
          ...
          { key: 'caseViewFontSize', value_json: 14 },
    ...
    const fontSizeSetting = execSettingsBody.find((item) => item.key === 'caseViewFontSize' && item.owner_id === userId);
    expect(fontSizeSetting && fontSizeSetting.value_json).toBe(14);
    ...
    expect(switchTheme.status()).toBe(200);
    const listThemeSettings = await ctx.get(`${apiBase}/api/settings?scope=all`, { headers: userHeaders });
    expect(listThemeSettings.status()).toBe(200);
