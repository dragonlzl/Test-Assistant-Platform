...
自动创建并清理临时 SQLite 数据库和备份目录，不连接正式库。覆盖首次生成、24 小时/7 天边界、每类只保留一份、重启与停机补做、不同库隔离、WAL 已提交/未提交数据、单文件完整性、复制与替换失败保留旧快照、超时、崩溃临时文件清理、多进程互斥、后台重试、应用启停、从快照恢复后继续读写及静态访问隔离。调度测试使用可控时钟，无需实际等待一天或一周。
