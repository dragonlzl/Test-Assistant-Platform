(function() {
  var caseWritingStyleGuidePrompt = [
    ...
    '14、提交新增前做复杂度自检：单条包含多个独立目标、互斥前置或成功/失败/取消等分支，过长复杂影响简易阅读时，先去掉冗余，再按独立测试目标拆成多条用例。每条聚焦一个主要检查点，前置、步骤、预期完整且可独立执行，不写“同上”“接上一条”。',
    '15、同一目标的必要连续操作与扣费/到账/次数刷新等关键校验点保留，不按固定字数机械拆分，不把复杂内容移入备注或省略覆盖点；对最终拆分候选逐条与已有用例及本批候选做语义去重。'
  ].join('\n');
