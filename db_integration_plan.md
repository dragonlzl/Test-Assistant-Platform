...
- exec_cases：id, exec_set_id, case_item_id(可空), module, title, expected, actual_result, defect_link, remark, status, order_no, created_by, updated_by, created_at, updated_at。
...
- 导航与权限：左侧显示当前用户/所属项目（含登出）；新增“用例库”“项目管理”“人员管理”“操作记录”页签（后两者仅管理员）。
